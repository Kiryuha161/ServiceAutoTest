...
for (let i = 0; i < localSites.length; i++) {
    const site = localSites[i];
    Scenario(`CreateSignatureRequest ${site}`, async ({ I }) => {
...
//#region Сервисы
Feature("Тест апи нейронки на определение контактов и мата");
Scenario('DefineContactsAndInvallidWordsTest', async ({ I }) => {
    const requestUrl = 'http://localhost:3000/sendPrompt';

    const body = {
        systemRoleText: 'Ты - анализатор текста, занимающийся автомодерацией. Тебе нужно определять, есть ли что-то, что не соответствует требованиям. Триггеры, на которые ты должен обращать внимания: неприличные слова, маты, контактные данные (телефонные номера, электронные почты, телеграм или whatsapp-каналы, и так далее). Если всё это встречается в тексте, то ответь - Да, если не встречается ответь - Нет. После этого поставь точку и одним словом напиши причину (например, мат или номер телефона). Если таких критерия больше одного, то опиши их, через запятую. Например: Да. Мат, телефонный номер. Или: Да. Мат. Или: Да. Телефонный номер.',
        userRoleText: JSON.stringify({
            Name: "Лот 1",
            Information: "Крутой лот, свяжитесь со мной по номеру +7 925 904 10 887",
            Rules: "Правила просты",
            DepositRules: "Мне всё до пизды"
        })
    };

    const response = await I.sendPostRequest(requestUrl, body);
    console.log(response.data["result"]["alternatives"][0]["message"]["text"]);
});

Feature("Тест апи нейронки на определение контактов в разной формате");
Scenario('DefineContactsTest', ({ I }) => {
    const requestUrl = 'http://localhost:3000/sendPrompt';
    ...
    phoneNumberFormats.forEach(async (format) => {
        const body = {
            //systemRoleText: 'Ты - анализатор текста, занимающийся автомодерацией. Тебе нужно определять, есть ли что-то, что не соответствует требованиям. Триггеры, на которые ты должен обращать внимания: неприличные слова, маты, контактные данные (телефонные номера, электронные почты, телеграм или whatsapp-каналы, и так далее). Если всё это встречается в тексте, то ответь - Да, если не встречается ответь - Нет. После этого поставь точку и одним словом напиши причину (например, мат или номер телефона). Если таких критерия больше одного, то опиши их, через запятую. Например: Да. Мат, телефонный номер. Или: Да. Мат. Или: Да. Телефонный номер.',
            systemRoleText: 'Ты - анализатор текста, занимающийся автомодерацией. Тебе нужно определять, есть ли что-то, что не соответствует требованиям. Триггеры, на которые ты должен обращать внимания: неприличные слова, маты, контактные данные (телефонные номера, электронные почты, телеграм или whatsapp-каналы, и так далее). Если это встречается в тексте, то ответь - Да, если не встречается ответь - Нет.',
            
            userRoleText: JSON.stringify({
                Name: "Лот 1",
                Information: `Крутой лот, просто чудесный ${format}, умопомрачительный, покупайте`,
                Rules: "Правила просты",
                DepositRules: "Мне всё до фонаря"
            })
        };
...
Feature("Тест апи нейронки на определение электронных почт в разном формате");
Scenario('DefineEmailsTest', ({ I }) => {
    const requestUrl = 'http://localhost:3000/sendPrompt';
    
    const emailsFormats = [
        'test@yandex.ru',
        'test @ yandex.ru',
        'test сабака yandex.ru'
    ]

    emailsFormats.forEach(async (format) => {
        const body = {
            //systemRoleText: 'Ты - анализатор текста, занимающийся автомодерацией. Тебе нужно определять, есть ли что-то, что не соответствует требованиям. Триггеры, на которые ты должен обращать внимания: неприличные слова, маты, контактные данные (телефонные номера, электронные почты, телеграм или whatsapp-каналы, и так далее). Если это встречается в тексте, то ответь - Да, если не встречается ответь - Нет. После этого поставь точку и одним словом напиши причину (например, мат или номер телефона). Если таких критерия больше одного, то опиши их, через запятую.', 
            systemRoleText: 'Ты - анализатор текста, занимающийся автомодерацией. Тебе нужно определять, есть ли что-то, что не соответствует требованиям. Триггеры, на которые ты должен обращать внимания: неприличные слова, маты, контактные данные (телефонные номера, электронные почты, телеграм или whatsapp-каналы, и так далее). Если это встречается в тексте, то ответь - Да, если не встречается ответь - Нет.',
            userRoleText: JSON.stringify({
                Name: "Лот 1",
                Information: `Крутой лот, просто чудесный ${format}, умопомрачительный, покупайте`,
                Rules: "Правила просты",
                DepositRules: "Такие вот дела"
            })
        };
...
Feature("Тест апи нейронки на определение каналов месседжера");
Scenario('DefineMessagersChannelTest', async ({ I }) => {
    const requestUrl = 'http://localhost:3000/sendPrompt';

    const format = '@ESSchtolts';
    const body = {
        systemRoleText: 'Ты - анализатор текста, занимающийся автомодерацией. Тебе нужно определять, есть ли что-то, что не соответствует требованиям. Триггеры, на которые ты должен обращать внимания: неприличные слова, маты, контактные данные (телефонные номера, электронные почты, телеграм или whatsapp-каналы, и так далее). Если это встречается в тексте, то ответь - Да, если не встречается ответь - Нет.',  //После этого поставь точку и одним словом напиши причину (например, мат или номер телефона). Если таких критерия больше одного, то опиши их, через запятую.
        userRoleText: JSON.stringify({
            Name: "Лот 1",
